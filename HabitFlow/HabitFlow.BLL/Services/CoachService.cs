    ...
    public class CoachService : ICoachService
    {
            ...
            Guid userId, CoachAdviceRequest request)
        {
            ...
            if (string.IsNullOrEmpty(this.geminiApiKey))
            {
                ...
                return this.Fallback(request.SessionType, ctx);
            }

            var prompt = this.BuildPrompt(ctx, request);
            ...
            try
            {
                ...
                {
                    this.logger.LogError("Gemini error {S}: {B}", resp.StatusCode,
      ...
      "Для чату — відповідай як людина, без формату. Коротко.";
        private string BuildPrompt(CoachContext c, CoachAdviceRequest req)
        {
                ...
                && !string.IsNullOrWhiteSpace(req.UserMessage))
            {
                ...
                sb.AppendLine("Відповідай ТІЛЬКИ на це питання. Якщо питання не про звичку — просто дай пряму відповідь.");
                sb.AppendLine();
            }
        ...
        private CoachAdviceResponse ParseResponse(string text)
        {
        ...
        private CoachAdviceResponse Fallback(CoachSessionType type, CoachContext c)
        {
            this.logger.LogWarning("Fallback для {T}", type);
